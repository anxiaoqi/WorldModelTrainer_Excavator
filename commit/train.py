...
net = LSTM(input_size=8, hidden_size=128, num_layers=32, lstm_output_size=3, lnn_input_size=8, lnn_hidden_size_1=128,
           lnn_hidden_size_2=256, lnn_hidden_size_3=128,  lnn_hidden_size_4=64, lnn_output_size=5).cuda()  # 定义模型
loss = nn.MSELoss()  # 定义损失函数
optimizer = torch.optim.SGD(net.parameters(), lr=0.1)  # 定义优化器
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=80000, gamma=0.1)
length = 20  # 序列长度
batch = 256  # batch尺寸
...
# 测试数据
data_test_list = []
path_folder = "/home/ubuntu/4T/mdx/2_setdata/test/*"
for folder_abs in glob.glob(path_folder):
    # print(folder_abs)
    path_file = folder_abs + "/*"
    for file_abs in glob.glob(path_file):
