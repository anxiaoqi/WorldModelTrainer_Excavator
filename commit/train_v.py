...
def extract_data(u, next_v, length, batch):  # define method to extract X and Y
    input_u = []
    output_next_v = []
    ...
    return u_prep, next_v_prep, start
...
data_next_max = np.array([6.16101226e-01, 9.44223125e-01, 1.06465084e+00])
data_next_min = np.array([-5.23598776e-01, -9.02160690e+00, -1.21125850e+00])
    ...
    for file_abs in glob.glob(path_file):
        ...
        contend_u = contend.loc[:,
                    ['pos_boom', 'pos_arm', 'pos_swing', 'vel_boom', 'vel_arm', 'vel_swing', 'state_time',
                     'action_time',
                     'pwm_boom', 'pwm_arm', 'pwm_swing']][0:-1]
        contend_next_v = contend.loc[:, ['next_vel_boom', 'next_vel_arm', 'next_vel_swing']][0:-1]

        # contend_u_noise = wgn(contend_u, 16)
        contend_u_norm = (contend_u - data_u_min) / (data_u_max - data_u_min)
        contend_next_v_norm = (contend_next_v - data_next_min) / (data_next_max - data_next_min)
        # data.info()
        # data_list.append(contend)

        u_list.append(contend_u_norm)
        next_v_list.append(contend_next_v_norm)
...
# net_v = LSTM_V(lstm_in=11, lstm_hid=256, lstm_layer=16, lstm_out=3, lnn_in=6,
...
optimizer = torch.optim.SGD(net_v.parameters(), lr=0.1)  # 定义优化器
# optimizer = torch.optim.Adam(net_v.parameters(), lr=1e-3, eps=1e-8)  # 定义优化器
...
for e in range(1500001):
    ...
    # print(page)
    in_u = u_list[page]
    in_next_v = next_v_list[page]

    if len(in_u) < length + batch:
        continue
    u_prep, next_x_prep, start = extract_data(in_u, in_next_v, length, batch)

    input_u = torch.tensor(u_prep, dtype=torch.float32).cuda()
    input_next_v = torch.tensor(next_x_prep, dtype=torch.float32).cuda()
    # pdb.set_trace()
    cal_next_v = net_v(input_u)  # 向前传播  (50, 1 , 3)
    # pdb.set_trace()
    Loss = 10 * loss(cal_next_v, input_next_v)  # 计算损失 #()

    if Loss < last_Loss:
        checkpoint = {'epoch': e,
                      'best_loss': Loss,
                      'model': net_v.state_dict(),
