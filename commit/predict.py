...
# 函数格式
def state_predict(states: torch.Tensor, actions: torch.Tensor, timesteps: torch.Tensor, mask: torch.Tensor, device,
    ...
    lstm_input_max = torch.tensor(
        [6.03883921e-01, 7.33038286e-01, 1.00880031e+00, 1.49962902e-01, 1.21906042e-01, 1.00000000e+03, 1.00000000e+03,
         1.00000000e+03]).unsqueeze(0).unsqueeze(0)
    lstm_input_min = torch.tensor(
        [-4.55530935e-01, -1.15191731e+00, -8.93608577e-01, 0.00000000e+00, 0.00000000e+00, -1.00000000e+03,
         -1.00000000e+03, -1.00000000e+03]).unsqueeze(0).unsqueeze(0)
    lstm_output_max = torch.tensor([6.03883921e-01, 7.33038286e-01, 1.00880031e+00]).unsqueeze(0).unsqueeze(0)
    lstm_output_min = torch.tensor([-4.55530935e-01, -1.15191731e+00, -8.93608577e-01]).unsqueeze(0).unsqueeze(0)
    lnn_input_max = torch.tensor(
        [6.49962359e-01, 1.25976120e+00, 3.14159265e+00, 1.49962902e-01, 1.21906042e-01]).unsqueeze(0).unsqueeze(0)
    lnn_input_min = torch.tensor(
        [-8.60271043e-01, -8.82630456e-01, -3.13984732e+00, 0.00000000e+00, 0.00000000e+00]).unsqueeze(0).unsqueeze(0)
    lnn_output_max = torch.tensor([6.49962359e-01, 1.25976120e+00, 3.14159265e+00, 1.49962902e-01, 1.21906042e-01]).unsqueeze(0).unsqueeze(0)
    lnn_output_min = torch.tensor([-8.60271043e-01, -8.82630456e-01, -3.13984732e+00, 0.00000000e+00, 0.00000000e+00]).unsqueeze(0).unsqueeze(0)
    ...
    net = torch.load('net_final.pth').to(device)
    # net.eval()
    # import pdb
    # pdb.set_trace()
    cal_next_v, cal_next_x = net(state_lstm_input, state_lnn_input)
    cal_next_v_out = cal_next_v.cpu().detach() * (lstm_output_max - lstm_output_min) + lstm_output_min
    cal_next_x_out = cal_next_x.cpu().detach() * (lnn_output_max - lnn_output_min) + lnn_output_min
